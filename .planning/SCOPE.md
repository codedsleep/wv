...
- [x] **4.2 Property-based parser tests** — *Codex* — `src/backend/tmux/parser.rs` (proptest module)
  - Add `proptest` as a dev-dependency
  - Property: feeding any randomly-generated byte sequence (split into 1–3 chunks across `feed` calls) must not panic and the parser must remain in a valid state
